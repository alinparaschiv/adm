...
<form action="index.php?link=asoc_dat" method="post">
<table cellspacing=5 style="margin:20px 0 0 0px; width:500px;" border=0>
	...
	<tr>
		...
		<td bgcolor="#CCCCCC">
			<select name="asoc_id" onchange="asoc_change(this.value)">
			<option disabled="disabled">Alege Asociatia</option>
			<?php
	...
	while($row = mysql_fetch_assoc($q)){ ?>
		<tr bgcolor="#<?php echo ($color++ % 2) == 0 ? 'CCCCCC' : '999999';?>">
			...
			<td><?php echo $row['penalizare']; ?></td>
                        <td>Edit <a href="index.php?link=asoc_dat&asoc_id=<?php echo $asocId; ?>&actiune=del&fur_id=<?php echo $row['fur_id']; ?>">Sterge</a> Salveaza</td>
		</tr>
	<?php }
	echo '</table>';
