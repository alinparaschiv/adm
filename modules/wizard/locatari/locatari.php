...
<script language="javascript">
        ...
        function infoScari(obj) {
            window.location = "index.php?link=w_locatari&asoc_id=<?php echo $asocId; ?>&scara="+obj;
        }
...
echo 'Nr scari gasite in urma queryului: '.mysql_num_rows($result);
...
if ($_POST['buton'] == 'apasat' && $_POST['nume'] != '' && $_POST['nr_pers'] !='' && $_POST['supr'] != '' && $_POST['cota']!='' && $_POST['ap_rece']!='' && $_POST['ap_calda']!='' && $_POST['centrala']!='nimic'  && $_POST['incalzire']!='nimic'  && $_POST['gaz']!='nimic'  && $_POST['ilum_lift']!='nimic'   && $_POST['service_lift']!='nimic' ) {
        $i = 0;
	...
	if ($tipIncalzire == 1) { //daca este selectat criteriul cu repartitoare
	        if(ereg('[^0-9]', $nr_rep) || $nr_rep=='') {
	              $i=1;
	              $nr_repErr = 'Campul "Nr rep" ('.$nr_rep.') poate sa contina doar cifre.<br>';
	        }
				}

        if ($i==0) {
            $next = 1;
            $query = "INSERT INTO locatari (`scara_id`,`asoc_id`, `nume`,`cnp`,`nr_pers`,`supr`,`cota`,`etaj`, `ap`, `ap_rece`, `ap_calda`, `tip_incalzire`, `nr_rep`, `centrala`, `incalzire`, `gaz`, `ilum_lift`, `service_lift`, `ap_locuit`) VALUES
                                           ('$scaraId', '$asocId', '$nume', '$cnp','$nr_pers','$supr','$cota','$etaj', '$apNr', '$ap_rece', '$ap_calda', '$tipIncalzire', '$nr_rep', '$centrala', '$incalzire', '$gaz', '$ilum_lift', '$service_lift', '$apLocuit')";
...
<?php
if ($link=='w_locatari') {
    echo '<form id="addForm" name="addForm" method="post" action="index.php?link=w_locatari&asoc_id='.$asocId.'&scara='.$scaraId.'">';
...
?>
                ...
                Scara:
                <?php
                        $query = "SELECT SS.*, S.scara FROM scari_setari AS SS LEFT JOIN scari AS S ON SS.scara_id=S.scara_id WHERE SS.asoc_id='$asocId' ORDER BY S.scara";
                        echo '<select name="scara" onchange="infoScari(this.value)">';
                                $result = mysql_query($query) or die('<br><br>A avut loc o eroare, contactati andreista@hotmail.com / 0744832335 ');
                                while($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
                                    if ($scaraId == $row['scara_id'] )  {
                ...
                <table cellspacing=1 style="margin:10px 0 0 0px; width:950px;" border=0>
                        <tr bgcolor="#19AF62">
                               ...
                               <td><font size=2 color="white"><center>Nume si Prenume</center></font></td>
							   ...
							   <td><font size=2 color="white"><center>Apartament<br />Locuit</center></font></td>
					   </tr>
                        <?php
                                if ($apNr %2 == 0) { echo '<tr align=center>';   }    else    {     echo '<tr align=center bgcolor="#f1f2f2">';    }
                                    echo '<td><select name="etaj">';
                                            ...
                                            for ($i=1; $i<=$etaje; $i++)  {	echo '<option value="'.$i.'">'.$i.'</option>'; }
									echo '</select></td>';
                                    ...
                                    echo '<td><select name="tipIncalzire" onchange="checkNr(this.value)">';
                                         echo '<option value="0" '. ( $_POST['tipIncalzire'] == 0 ? 'selected="selected"' : '' ).' >Nimic</option>';
                                         echo '<option value="1" '. ( $_POST['tipIncalzire'] == 1 ? 'selected="selected"' : '' ).' >Repartitoare</option>';
                                         echo '<option value="2" '. ( $_POST['tipIncalzire'] == 2 ? 'selected="selected"' : '' ).' >Gigacalorimetru</option>';
                                    echo '</select></td>';

									# NR REPARTITOARE
                                    echo '<td><input style="width:30px;" id="nr_rep" maxlength="2" name="nr_rep" value="'.$_POST['nr_rep'].'" disabled></td>';
									...
									echo '<td ><select name="centrala"><option value="nimic">Alege</option>';
                                                ...
                                                echo '<option value="CT">da</option><option value="nu">nu</option>';
                                            echo '</select></td>';
								  ...
								  # ILUMINARE LIFT
								   ...
								   echo '<td ><select name="service_lift"><option value="nimic">Alege</option>';
									...
									# APARTAMENT LOCUIT
									if ($_POST['apLocuit'] == "on"){
										echo '<td ><input type="checkbox" name="apLocuit" checked/></td>';
									} else {
										echo '<td ><input type="checkbox" name="apLocuit" /></td>';
									}
                                echo '</tr>';
					   ...
					   if ($mesaj != '') echo '<tr><td></td><td>'.$mesaj.'</td></tr>'; ?>
                        </table>
...
<?php
    ...
    if ($_POST['salveaza'] == 'apasat') {
            ...
            if ($tipIncalzireT == 1) { //daca este selectat criteriul cu repartitoare
	        if(ereg('[^0-9]', $nr_repT) || $nr_repT=='') {
	              $i=1;
	              $nr_repErr = 'Campul "Nr rep" ('.$nr_repT.') poate sa contina doar cifre.<br>';
	        }
	    }
            ...
            if ($i == 0) {
                    ...
                    `service_lift`='$service_liftT', `ap_locuit`='$apLocuitT' WHERE loc_id='$lcLd'";
              ...
              while($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
                   ...
                   if ($i %2 == 0) echo '<tr align=center>'; else echo '<tr align=center bgcolor="#f1f2f2">';
                        ...
                        if ($editLoc == $row['loc_id']){
                                echo '<form name="formEdit" id="formEdit" method="post" action="index.php?link=w_locatari&asoc_id='.$asocId.'&scara='.$scaraId.'">';
                                    ...
                                    echo '<td ><select name="service_liftT">';
                                            ...
                                            echo '</select></td>';
                                if ($row['ap_locuit'] == 1){
                                        echo '<td><input type="checkbox" name="apLocuitT" value="on" checked/></td>';
                                } else {
                                        echo '<td><input type="checkbox" name="apLocuitT" value="on" /></td>';
                                }
                                ...
                                echo '</form>';
                                    ...
                                    echo '<td><center>
                                                <a style="font-size:12px;" href="index.php?link=w_locatari&asoc_id='.$asocId.'&edit='.$row['loc_id'].'&scara='.$scaraId.'">[edit]</a>&nbsp;&nbsp;&nbsp;
                                          </center></td>';
                        }
                   echo '</tr>';
                   $i++;
