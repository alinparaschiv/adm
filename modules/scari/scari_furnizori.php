...
if (isset($_POST['actiune']) && $_POST['actiune'] == "Adauga Furnizor") {
	...
	if ($i == 0) {
        ...
        $q1 = "SELECT min(fact_id) id FROM fisa_furnizori";
		$q1 = mysql_query($q1) or die("Nu pot gasi un id de factura valid <br />".mysql_error());
		$factId = mysql_result($q1, 0, 'id') - 1;
...
function asoc_change(val){
	window.location = 'index.php?link=scari_furnizori&asoc_id=' + val;
}
function scara_change(sc){
	window.location = 'index.php?link=scari_furnizori&asoc_id=' + sc[sc.selectedIndex].id + '&scara_id=' + sc.value;
}
</script>

<form action="index.php?link=scari_furnizori" method="post">
<table cellspacing=5 style="margin:20px 0 0 0px; width:500px;" border=0>
	...
	<tr>
		<td bgcolor="#CCCCCC">Asociatia:</td>
		<td bgcolor="#CCCCCC">
			<select name="asoc_id" onchange="asoc_change(this.value)"><?php
			$q = "SELECT * FROM asociatii"." ORDER BY administrator_id, asoc_id";
			$q = mysql_query($q) or die("Nu pot selecta asociatiile".mysql_error());
			while($row = mysql_fetch_assoc($q)){
				echo '<option value="'.$row['asoc_id'].'" '. ((isset($_GET['asoc_id']) && ($_GET['asoc_id'] == $row['asoc_id']) || (isset($_POST['asoc_id']) && ($_POST['asoc_id'] == $row['asoc_id']))) ? 'selected="selected"' : '' ).'>'.$row['asociatie'].'</option>';
			}
			?></select>
		</td>
	</tr>
	<tr>
		...
		<td bgcolor="#CCCCCC">
			<select name="scara_id" onchange="scara_change(this)"><?php
			$q = "SELECT S.*, A.asociatie FROM scari S, asociatii A WHERE S.asoc_id=A.asoc_id ";
			if($asoc != null) $q .= 'AND A.asoc_id='.$asoc;
			$q = mysql_query($q." ORDER BY A.administrator_id, A.asoc_id") or die("Nu pot selecta asociatiile".mysql_error());
			if (mysql_num_rows($q) == 1) {
				...
				echo '<option disabled="disabled">Alege Scara</option>';
			}
			while($row = mysql_fetch_assoc($q)){
				echo '<option value="'.$row['scara_id'].'" id="'.$row['asoc_id'].'" '. ((isset($_GET['scara_id']) && ($_GET['scara_id'] == $row['scara_id']) || (isset($_POST['scara_id']) && ($_POST['scara_id'] == $row['scara_id']))) ? 'selected="selected"' : '' ).'>'.$row['scara'].' ('.$row['asociatie'].')</option>';
			}
