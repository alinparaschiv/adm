...
$termenCitire = "SELECT * FROM asociatii_setari a_s, asociatii a WHERE a.asoc_id=a_s.asoc_id AND a.asoc_id=".$asocId;
$termenCitire = mysql_query($termenCitire) or die("#1 -- Nu am putut selecta termenul de predare a citirilor<br />" . mysql_error());

while ($ultimaZi = mysql_fetch_array($termenCitire)) {//o sa se execute numai o singura data
	...
	$nrLuni = mysql_query($nrLuni) or die("#1.1 - Nu pot afla cate luni sunt in tabela apometre pentru asociatia curenta<br />" . mysql_error());
    $nrLuni = mysql_num_rows($nrLuni);
    //daca ieri a fost termenul limita
    //verific daca au dat toti citirile
    //pentru luna anterioara
    if ((strtotime($dataIeri) <= strtotime($dataTermen)) && ($nrLuni <= 0)) {
        echo '<br />Data ieri = Data termen<br />';
        //aflu cate luni sunt in tabela apometre
        $nrLuni = "SELECT * FROM apometre WHERE asoc_id=" . $ultimaZi['asoc_id'] . " AND luna<='" . $lunaAnt . "' GROUP BY luna";
        $nrLuni = mysql_query($nrLuni) or die("#1.2 - Nu pot afla cate luni sunt in tabela apometre pentru asociatia curenta<br />" . mysql_error());
        $nrLuni = mysql_num_rows($nrLuni);
                ...
                if ($parcurgLocatari['repetari'] != 0) {
                    ...
                    switch ($ultimaZi['impartire2']) {
                        ...
                        case 1:  //pausal cu modificarea indexului
                            ...
                            $rPApoCald = $pausalCald % $nrApoCald;
                            $cPApoCald = ($pausalCald - $rPApoCald) / $nrApoCald;

                            $rPApoRece = $pausalRece % $nrApoRece;
                            $cPApoRece = ($pausalRece - $rPApoRece) / $nrApoRece;
