<?php
//Dupa spusele lui Casian fondul de reparati trebuie sa se poata inparti pe suprafata si pe ap asa ca daca se inparte pe ap (sum -> count)

$inpartiePeSuprafata = intval($proc['mod_impartire']) == 0 ? false : true;

echo '<br />Facturade F. Rep. se imparte pe '.( $inpartiePeSuprafata ? 'sup' : 'ap' );
...
if (mysql_num_rows($sql1) != 0) {
    while ($row = mysql_fetch_array($sql1)) {
        ...
        $fondRulment = "UPDATE fisa_fonduri SET fond_rep_rest=".$aveaDePlata." WHERE  loc_id=".$row['loc_id']." AND data='".$lunaUrm."'";
        $fondRulment = mysql_query($fondRulment) or die ("Nu pot updata fondul de reparatii<br />".mysql_error());

        //$fond_rul_s = 'SELECT * FROM fisa_fond WHERE loc_id='.$row['loc_id'].' AND STR_TO_DATE(data, "%m-%Y") > STR_TO_DATE("'.$luna.'"", "%m-%Y")';
        //$fond_rul_q = mysql_query($fond_rul_s) or die('Nu pot afla urmatoarele inregistrari din fisa fond ce trebuie actualizate');
    ...
    while ($amLocatar = mysql_fetch_array($iiDamDePlata)) {
        ...
        $fondRepRest = round($ppu * ( $inpartiePeSuprafata ? $amLocatar['supr'] : 1 ) * $amLocatar['sub'] / 100, 2) + $fondRepRest;
