...
<sethtmlpageheader name="urb_fond_rep" page="ALL" value="1" show-this-page="1"/>
';
...
<table width="1100" border="0" cellpadding="1">
  ...
  <tr bgcolor="#666666">
    ...
    <td color="#FFFFFF" align="center"><strong>Suna cheltuita</strong></td>
  </tr>
  </thead>';
  
  $sql_incasat = "SELECT sum(fond_rep_incasat) as incasat
FROM fisa_fonduri WHERE asoc_id=".$_GET['asoc_id']." AND STR_TO_DATE(data, '%m-%Y') < STR_TO_DATE('".$_GET['luna']."', '%m-%Y')";
  ...
  $sql_cheltuit = "SELECT sum(fond_rep_cheltuit) as cheltuit
FROM fisa_fonduri WHERE asoc_id=".$_GET['asoc_id']." AND STR_TO_DATE(data, '%m-%Y') < STR_TO_DATE('".$_GET['luna']."', '%m-%Y')";
  $qery_cheltuit = mysql_query($sql_cheltuit) or die("Nu am putut afla suma raportata incasata<br />".$sql_cheltuit."<br />".mysql_error());
  
  
  $sql_restanta = "SELECT sum(fond_rep_rest) as restanta
					FROM fisa_fonduri 
  ...
  $total = array(mysql_result($qery_restanta,0,'restanta')+mysql_result($qery_incasat,0,'incasat'),
				 ...
				 mysql_result($qery_cheltuit,0,'cheltuit'));
  
  $sume_initiale = array($total[0], $total[1], $total[2]);
  
  $sql_loc = "SELECT L.loc_id, L.nume, F.fond_rep_incasat as i, F.fond_rep_rest as r, F.fond_rep_cheltuit as c
...
while($row = mysql_fetch_assoc($query_loc)) {
	...
	$color = ($color+1) % 2;
	 ...
	 if($row['i'] == null)
		...
		$sql_last = "SELECT fond_rep_incasat as i, fond_rep_rest as r FROM fisa_fonduri WHERE loc_id=".$row['loc_id']." AND
					STR_TO_DATE(data, '%m-%Y') < STR_TO_DATE('".$_GET['luna']."', '%m-%Y')
	 ...
	 $sql_chitante = "SELECT date_format(data_inserarii, '%d.%m.%Y') as data, concat(C.chitanta_serie,'/',C.chitanta_nr) as document, suma
					FROM `casierie` C WHERE loc_id=".$row['loc_id']." AND tip_plata='Fond Reparatii' AND
...
$ResultsPDFContent .=' 
  <tr>
    ...
    <td align="left"><strong>'.$total[1].'</strong></td>
	<td align="left"><strong>'.$total[2].'</strong></td>
  </tr>
</table>';
